...
import { Region, RegionUncheckedUpdateInput } from '../../gql/graphql'
...
const UPDATE_REGION = gql`
  mutation updateRegion($id: Float!, $name: String!) {
...
export default function useRegionsService() {
  ...
  const { data, loading, error } = useQuery<{ regions: Region[] }>(REGIONS)
