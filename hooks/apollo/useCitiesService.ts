...
const CREATE_CITY = gql`
  mutation createCity($name: String!, $regionId: Int!) {
    createCity(createCityInput: { name: $name, regionId: $regionId }) {
      id
      name
    }
  }
...
const UPDATE_CITY = gql`
  mutation updateCity($id: Int!, $name: String!, $regionId: Int!) {
    updateCity(
      id: $id
      input: { name: { set: $name }, regionId: { set: $regionId } }
    ) {
      id
      name
    }
...
export default function useCitiesService(regionId?: number | null) {
  ...
  const { data, loading, error } = useQuery<
    { cities: City[] },
    { regionId?: number }
  >(CITIES, { variables: { regionId: regionId ?? undefined } })
